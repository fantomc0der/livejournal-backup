...
> **Shortcut:** the examples below use `bun run src/index.ts` for clarity, but the repo ships with `cli.sh` and `cli.ps1` wrappers that invoke the CLI directly using an absolute path to `src/index.ts`, so they work from any working directory. You can replace `bun run src/index.ts` with `./cli.sh` (bash) or `.\cli.ps1` (PowerShell) in any command — e.g. `./cli.sh archive --year 2002`.
