...
5. Claude posts inline comments on the diff and a summary comment that ends with either `REVIEW: PASS` or `REVIEW: FAIL`. The summary comment is **sticky** — `claude-review.yml` is configured with `use_sticky_comment: true`, so re-reviews on subsequent pushes update the existing comment in place rather than creating a new one. After Claude finishes, a follow-up step in the workflow reads the latest verdict and exits non-zero on `REVIEW: FAIL`, so the PR's check rollup turns red on FAIL and green on PASS — don't rely on the comment looking "new" to know whether Claude re-reviewed; check the comment's updated timestamp and the workflow's check status.
